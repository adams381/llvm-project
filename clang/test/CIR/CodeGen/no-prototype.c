...
int test2(int x) {
  ...
  // CHECK:  {{.*}} = cir.call [[GGO]](%{{[0-9]+}}) {arg_attrs = [{llvm.noundef}]} : (!cir.ptr<!cir.func<(!s32i) -> !s32i>>, !s32i) -> !s32i
}
...
// CHECK: cir.func {{.*}} @test3
  ...
  // CHECK:  {{%.*}} = cir.call [[CAST]](%{{[0-9]+}}) {arg_attrs = [{llvm.noundef}]} : (!cir.ptr<!cir.func<(!s32i) -> !s32i>>, !s32i) -> !s32i
}
...
int test4(int x) {
  ...
  // CHECK:  [[CAST:%.*]] = cir.cast bitcast [[GGO]] : !cir.ptr<!cir.func<() -> !s32i>> -> !cir.ptr<!cir.func<(!s32i) -> !s32i>>
  // CHECK:  {{%.*}} = cir.call [[CAST]]({{%.*}}) {arg_attrs = [{llvm.noundef}]} : (!cir.ptr<!cir.func<(!s32i) -> !s32i>>, !s32i) -> !s32i
}
...
int test5(int x) {
  ...
  // CHECK:  {{%.*}} = cir.call [[CAST]]() {arg_attrs = []} : (!cir.ptr<!cir.func<() -> !s32i>>) -> !s32i
}
