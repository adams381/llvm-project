...
extern "C" void acc_loop(int *A, int *B, int *C, int N) {
  // CHECK: cir.func{{.*}} @acc_loop(%[[ARG_A:.*]]: !cir.ptr<!s32i> {llvm.noundef} loc{{.*}}, %[[ARG_B:.*]]: !cir.ptr<!s32i> {llvm.noundef} loc{{.*}}, %[[ARG_C:.*]]: !cir.ptr<!s32i> {llvm.noundef} loc{{.*}}, %[[ARG_N:.*]]: !s32i {llvm.noundef} loc{{.*}}) {
