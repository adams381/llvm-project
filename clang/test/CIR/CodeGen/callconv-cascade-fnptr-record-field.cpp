...
int caller(Holder h, int x) {
  D d = {x};
  return h.fp(d);
}

// Cascade target: Holder's fp field's pointee FuncType has been rewritten
// from (D)->int to (i32)->int.
...
// CIR-LABEL: cir.func {{.*}}@_Z6caller6Holderi
// LLVM:      define {{.*}} i32 @_Z6caller6Holderi(
// OGCG:      define {{.*}} i32 @_Z6caller6Holderi(
