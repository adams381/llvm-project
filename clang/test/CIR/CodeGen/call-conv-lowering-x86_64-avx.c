...
// RUN: FileCheck --check-prefix=LLVM-CIR-COMPAT23 --input-file=%t-compat23-cir.ll %s
// RUN: %clang_cc1 -triple x86_64-unknown-linux-gnu -fclang-abi-compat=23 -emit-llvm %s -o %t-compat23.ll
// RUN: FileCheck --check-prefix=LLVM-OGCG-COMPAT23 --input-file=%t-compat23.ll %s
...
// the rule pins them back to the command-line level.
...
// LLVM-CIR-COMPAT23: define dso_local void @take_v256_tgt(ptr noalias noundef byval(<8 x float>) align 32 %{{.+}})
// LLVM-OGCG-COMPAT23: define dso_local void @take_v256_tgt(ptr noundef byval(<8 x float>) align 32 %{{.+}})
...
// LLVM-CIR-COMPAT23: define dso_local void @take_v512_tgt(ptr noalias noundef byval(<16 x float>) align 64 %{{.+}})
// LLVM-OGCG-COMPAT23: define dso_local void @take_v512_tgt(ptr noundef byval(<16 x float>) align 64 %{{.+}})

// A call site has to agree with the callee it resolves to.  Both sides need
// the attribute here: passing an AVX vector to a function whose level does not
// admit it is ill-formed.
...
// from the function containing the call.  Sema does not reject this the way it
// rejects a direct cross-level call, so nothing else pins it.
