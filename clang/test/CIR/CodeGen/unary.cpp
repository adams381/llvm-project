...
// CHECK: cir.func{{.*}} @_Z3up0v() -> (!u32i {llvm.noundef})
...
// CHECK: cir.func{{.*}} @_Z3um0v() -> (!u32i {llvm.noundef})
// CHECK:   %[[A:.*]] = cir.alloca !u32i, !cir.ptr<!u32i>, ["a", init]
// CHECK:   %[[INPUT:.*]] = cir.load{{.*}} %[[A]]
...
// CHECK: cir.func{{.*}} @_Z3un0v() -> (!u32i {llvm.noundef})
// CHECK:   %[[A:.*]] = cir.alloca !u32i, !cir.ptr<!u32i>, ["a", init]
// CHECK:   %[[INPUT:.*]] = cir.load{{.*}} %[[A]]
...
// CHECK: cir.func{{.*}} @_Z4inc0v() -> (!s32i {llvm.noundef})
...
// CHECK: cir.func{{.*}} @_Z4dec0v() -> (!s32i {llvm.noundef})
// CHECK:   %[[A:.*]] = cir.alloca !s32i, !cir.ptr<!s32i>, ["a", init]
// CHECK:   %[[ATMP:.*]] = cir.const #cir.int<1> : !s32i
// CHECK:   cir.store{{.*}} %[[ATMP]], %[[A]] : !s32i
// CHECK:   %[[INPUT:.*]] = cir.load{{.*}} %[[A]]
...
// CHECK: cir.func{{.*}} @_Z4inc1v() -> (!s32i {llvm.noundef})
// CHECK:   %[[A:.*]] = cir.alloca !s32i, !cir.ptr<!s32i>, ["a", init]
// CHECK:   %[[ATMP:.*]] = cir.const #cir.int<1> : !s32i
// CHECK:   cir.store{{.*}} %[[ATMP]], %[[A]] : !s32i
// CHECK:   %[[INPUT:.*]] = cir.load{{.*}} %[[A]]
...
// CHECK: cir.func{{.*}} @_Z4dec1v() -> (!s32i {llvm.noundef})
// CHECK:   %[[A:.*]] = cir.alloca !s32i, !cir.ptr<!s32i>, ["a", init]
// CHECK:   %[[ATMP:.*]] = cir.const #cir.int<1> : !s32i
// CHECK:   cir.store{{.*}} %[[ATMP]], %[[A]] : !s32i
// CHECK:   %[[INPUT:.*]] = cir.load{{.*}} %[[A]]
...
// CHECK: cir.func{{.*}} @_Z4inc2v() -> (!s32i {llvm.noundef})
// CHECK:   %[[A:.*]] = cir.alloca !s32i, !cir.ptr<!s32i>, ["a", init]
...
// CHECK: cir.func{{.*}} @_Z6fpPlusv() -> (!cir.float {llvm.noundef})
...
// CHECK: cir.func{{.*}} @_Z7fpMinusv() -> (!cir.float {llvm.noundef})
// CHECK:   %[[A:.*]] = cir.alloca !cir.float, !cir.ptr<!cir.float>, ["a", init]
// CHECK:   %[[INPUT:.*]] = cir.load{{.*}} %[[A]]
// CHECK:   %[[OUTPUT:.*]] = cir.unary(minus, %[[INPUT]])
...
// CHECK: cir.func{{.*}} @_Z8fpPreIncv() -> (!cir.float {llvm.noundef})
// CHECK:   %[[A:.*]] = cir.alloca !cir.float, !cir.ptr<!cir.float>, ["a", init]
...
// CHECK: cir.func{{.*}} @_Z8fpPreDecv() -> (!cir.float {llvm.noundef})
// CHECK:   %[[A:.*]] = cir.alloca !cir.float, !cir.ptr<!cir.float>, ["a", init]
// CHECK:   %[[ATMP:.*]] = cir.const #cir.fp<1.000000e+00> : !cir.float
// CHECK:   cir.store{{.*}} %[[ATMP]], %[[A]] : !cir.float
// CHECK:   %[[INPUT:.*]] = cir.load{{.*}} %[[A]]
...
// CHECK: cir.func{{.*}} @_Z9fpPostIncv() -> (!cir.float {llvm.noundef})
// CHECK:   %[[A:.*]] = cir.alloca !cir.float, !cir.ptr<!cir.float>, ["a", init]
// CHECK:   %[[ATMP:.*]] = cir.const #cir.fp<1.000000e+00> : !cir.float
// CHECK:   cir.store{{.*}} %[[ATMP]], %[[A]] : !cir.float
// CHECK:   %[[INPUT:.*]] = cir.load{{.*}} %[[A]]
...
// CHECK: cir.func{{.*}} @_Z9fpPostDecv() -> (!cir.float {llvm.noundef})
// CHECK:   %[[A:.*]] = cir.alloca !cir.float, !cir.ptr<!cir.float>, ["a", init]
// CHECK:   %[[ATMP:.*]] = cir.const #cir.fp<1.000000e+00> : !cir.float
// CHECK:   cir.store{{.*}} %[[ATMP]], %[[A]] : !cir.float
// CHECK:   %[[INPUT:.*]] = cir.load{{.*}} %[[A]]
...
// CHECK: cir.func{{.*}} @_Z10fpPostInc2v() -> (!cir.float {llvm.noundef})
// CHECK:   %[[A:.*]] = cir.alloca !cir.float, !cir.ptr<!cir.float>, ["a", init]
...
// CHECK: cir.func{{.*}} @_Z9fp16UPlusDF16_({{.*}}) -> (!cir.f16 {llvm.noundef})
...
// CHECK: cir.func{{.*}} @_Z10fp16UMinusDF16_({{.*}}) -> (!cir.f16 {llvm.noundef})
// CHECK:   %[[INPUT:.*]] = cir.load{{.*}} %[[F:.*]]
// CHECK:   %[[PROMOTED:.*]] = cir.cast floating %[[INPUT]] : !cir.f16 -> !cir.float
