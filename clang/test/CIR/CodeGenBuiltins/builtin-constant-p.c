...
// `struct foo` has two `int` fields — 8 bytes total — so the ABI
// classifies the return as a single integer eightbyte and CIR coerces
// the return type to `!s64i` (matching OGCG's `i64`).
