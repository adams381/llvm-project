...
// The calling-convention lowering pass coerces each `dim3` (12 bytes:
// 1.5 eightbytes) into a `(i64, i32)` pair — matching OGCG's ABI.
...
int main(void) {
  ...
  // CUDA-NEW: cir.call @__cudaPushCallConfiguration({{.*}}) : (!u64i, !u32i{{.*}}, !u64i{{.*}}, !u32i{{.*}}, !u64i, !cir.ptr<!rec_cudaStream>) -> !s32i
  // HIP-NEW: cir.call @__hipPushCallConfiguration({{.*}}) : (!u64i, !u32i{{.*}}, !u64i{{.*}}, !u32i{{.*}}, !u64i, !cir.ptr<!rec_hipStream>) -> !u32i
  //
