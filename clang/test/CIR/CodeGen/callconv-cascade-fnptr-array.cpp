...
int caller(int idx, int x) {
  int (*table[3])(D) = {worker, worker, worker};
  D d = {x};
  return table[idx](d);
}

// Cascade target: array element type carries the rewritten FuncType.
// CIR-DAG: cir.global {{.*}}@__const._Z6callerii.table = #cir.const_array<{{.*}}> : !cir.array<!cir.ptr<!cir.func<(!s32i) -> !s32i>> x 3>
