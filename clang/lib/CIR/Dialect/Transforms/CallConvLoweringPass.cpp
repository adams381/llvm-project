...
// aggregates, `_Complex`, and fixed-width vectors.  Scalable vectors, packed or
// padded records, and a union no member of which spans its declared size are
...
/// Also a `_Complex` or fixed-width vector of a supported element type.
...
convertABIArgInfo(const llvm::abi::ArgInfo &info, MLIRContext *ctx,
  ...
  if (info.isDirect()) {
    ...
    // Unreachable from the types isSupportedType admits: every coercion the
    // x86_64 classifier builds maps.  Kept because reading a null as "no
    // change" would pass the value unchanged and silently wrong.
...
static std::optional<FunctionClassification> classifyX86_64Signature(
  ...
  // bridge cannot represent.  No signature reaches this today.
...
/// The AVX level in force at \p func, which decides whether a vector wider
/// than 128 bits reaches a register.  `__attribute__((target("avx")))` raises
/// it above the level the command line set, so two functions in one module can
/// classify the same vector differently.  Follows
/// getEffectiveX86AVXABILevel in clang/lib/CodeGen/Targets/X86.cpp, which reads
/// the attribute itself.  Here the feature list CIRGen recorded stands in for
/// it: a function without the attribute records the command line's features,
/// which is the level \p base already carries.
static llvm::abi::X86AVXABILevel funcAvxLevel(cir::FuncOp func,
  ...
  // Test the wider name first: avx512f implies avx, and either order of
  // entries in the list has to give the higher of the two.
  if (enabled("avx512f"))
...
struct CallConvLoweringPass
  ...
  /// struct has no command-line parser.
...
void CallConvLoweringPass::runOnOperation() {
  ...
  std::array<std::unique_ptr<llvm::abi::TargetInfo>, 3> x86Targets;
  if (isX86)
    x86TypeMapper.emplace(dl);
  auto x86TargetFor =
      [&](llvm::abi::X86AVXABILevel level) -> const llvm::abi::TargetInfo & {
    assert(static_cast<unsigned>(level) < x86Targets.size() &&
           "one classifier slot per X86AVXABILevel enumerator");
  ...
  auto avxLevelFor = [&](cir::FuncOp func) {
  ...
  moduleOp.walk([&](Operation *op) {
    ...
    // and its call sites from one classification, so they have to agree.  A
    // call that would observe the difference is a Sema error.
    std::optional<FunctionClassification> fc =
  ...
  for (cir::CIRCallOpInterface c : indirectCalls) {
    ...
    auto classifySignature =
      ...
      // level comes from the function containing the call, as classic CodeGen
      // arranges every call site from the caller's declaration.
      if (isX86)
        return classifyX86_64Signature(
