...
// `S` has a user-defined destructor, so per the Itanium C++ ABI it is
// passed by pointer (caller-destructed convention) — the same shape
// classic Clang emits.  Matches OGCG's `(ptr noundef %a, ptr noundef %b)`.
