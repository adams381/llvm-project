...
__m128bh test_mm256_mask_cvtneps_pbh(__m128bh src, __mmask8 k, __m256 a) {
  ...
  // CIR: cir.call @_mm256_mask_cvtneps_pbh({{.+}}, {{.+}}, {{.+}}) {nobuiltin, nobuiltins = [{{.*}}]} : (!cir.vector<8 x !u16i>, !u8i, !cir.vector<8 x !cir.float>) -> !cir.vector<8 x !u16i>
...
__m128bh test_mm256_maskz_cvtneps_pbh(__mmask8 k, __m256 a) {
  ...
  // CIR: cir.call @_mm256_maskz_cvtneps_pbh({{.+}}, {{.+}}) {nobuiltin, nobuiltins = [{{.*}}]} : (!u8i, !cir.vector<8 x !cir.float>) -> !cir.vector<8 x !u16i>
...
__m128bh test_mm_mask_cvtneps_pbh(__m128bh src, __mmask8 k, __m128 a) {
  ...
  // CIR: cir.call @_mm_mask_cvtneps_pbh({{.+}}, {{.+}}, {{.+}}) {nobuiltin, nobuiltins = [{{.*}}]} : (!cir.vector<8 x !u16i>, !u8i, !cir.vector<4 x !cir.float>) -> !cir.vector<8 x !u16i>{{.+}}
...
__m128bh test_mm_maskz_cvtneps_pbh(__mmask8 k, __m128 a) {
  ...
  // CIR: cir.call @_mm_maskz_cvtneps_pbh({{.+}}, {{.+}}) {nobuiltin, nobuiltins = [{{.*}}]} : (!u8i, !cir.vector<4 x !cir.float>) -> !cir.vector<8 x !u16i>
