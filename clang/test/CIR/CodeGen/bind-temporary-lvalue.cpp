...
// CIR:   cir.call @_Z4getSv(%[[TMP]]) : (!cir.ptr<!rec_S> {{.*}}) -> ()
...
// LLVM:        call void @_Z4getSv(ptr {{.*}} sret(%struct.S) {{.*}} %[[TMP]])
// LLVM:        %[[X:.*]] = getelementptr inbounds {{.*}} %struct.S, ptr %[[TMP]], i32 0, i32 0
// LLVM:        %[[VAL:.*]] = load i32, ptr %[[X]]
// LLVMCIR:     call void @_ZN1SD1Ev(ptr {{.*}} dereferenceable(4) %[[TMP]])
// OGCG:        call void @_ZN1SD1Ev(ptr {{.*}} dead_on_return(4) dereferenceable(4) %[[TMP]])
// LLVM:        ret i32
