...
void use(int x, unsigned int y, float f, ConvertsToScalar cts) {
  // The CallConvLowering pass drops the empty `ConvertsToScalar` parameter
  // (Ignore class per x86_64 SysV, matching classic Clang).  The body still
  // references the by-value object via a synthesized `["ignored"]`
  // alloca + load that feeds the local `cts`.
  // CHECK: cir.func{{.*}}(%[[X_ARG:.*]]: !s32i{{.*}}, %[[Y_ARG:.*]]: !u32i{{.*}}, %[[F_ARG:.*]]: !cir.float{{.*}}) {{.*}}{
  // CHECK-NEXT: %[[CTS_IGNORED_ALLOC:.*]] = cir.alloca !rec_ConvertsToScalar, !cir.ptr<!rec_ConvertsToScalar>, ["ignored"]
  // CHECK-NEXT: %[[CTS_IGNORED_LOAD:.*]] = cir.load %[[CTS_IGNORED_ALLOC]] : !cir.ptr<!rec_ConvertsToScalar>, !rec_ConvertsToScalar
  ...
  // CHECK-NEXT: cir.store %[[CTS_IGNORED_LOAD]], %[[CTS_ALLOC]] : !rec_ConvertsToScalar, !cir.ptr<!rec_ConvertsToScalar>
  ...
  // CHECK-NEXT: %[[MUL:.*]] = cir.mul %[[Y_TO_FLOAT]], %[[F_LOAD]] : !cir.float
