...
// CIR: cir.func{{.*}} @printf(!cir.ptr<!s8i> {llvm.noalias, llvm.noundef}, ...) -> !s32i
...
// LLVM:   %[[printf_result2:.+]] = call i32 (ptr, ...) @printf(ptr noalias noundef @.str, ptr %[[str_val]])
// LLVM:   %[[str_val2:.+]] = load ptr, ptr %[[str_ptr]]{{.*}}
// LLVM:   %[[i_val:.+]] = load i32, ptr %[[i_ptr]]{{.*}}
// LLVM:   %[[printf_result3:.+]] = call i32 (ptr, ...) @printf(ptr noalias noundef @.str.1, ptr %[[str_val2]], i32 %[[i_val]])
// LLVM:   ret void
