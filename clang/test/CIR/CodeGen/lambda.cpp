...
//       CIR and LLVM coerce the single-pointer lambda to a pointer return.
...
// CIR:   %[[CAST:.*]] = cir.cast bitcast %[[COERCE]] : !cir.ptr<![[REC_LAM_G2]]> -> !cir.ptr<!cir.ptr<!void>>
// CIR:   %[[RET:.*]] = cir.load{{.*}} %[[CAST]] : !cir.ptr<!cir.ptr<!void>>, !cir.ptr<!void>
// CIR:   cir.return %[[RET]] : !cir.ptr<!void>
...
// CIR:   %[[CAST:.*]] = cir.cast bitcast %[[COERCE]] : !cir.ptr<!cir.ptr<!void>> -> !cir.ptr<![[REC_LAM_G2]]>
// CIR:   %[[LAM:.*]] = cir.load{{.*}} %[[CAST]] : !cir.ptr<![[REC_LAM_G2]]>, ![[REC_LAM_G2]]
...
// CIR:   %[[THIS_ADDR:.*]] = cir.alloca "this" {{.*}} init : !cir.ptr<!cir.ptr<![[REC_LAM_PTR_A]]>>
// CIR:   %[[RETVAL:.*]] = cir.alloca "__retval" {{.*}} : !cir.ptr<!s32i>
// CIR:   cir.store{{.*}} %[[THIS_ARG]], %[[THIS_ADDR]]
// CIR:   %[[THIS:.*]] = cir.load{{.*}} %[[THIS_ADDR]]
