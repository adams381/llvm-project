...
// CIR-SAME:    (%[[RET_ARG:.*]]: !cir.ptr<!rec_S> {{.*}}, %[[I_ARG:.*]]: !s32i {{.*}})
...
// CIR-NOT:     cir.copy
// CIR:         cir.call @_ZZ11agg_invokervENK3$_0clEi(%[[RET_ARG]], %[[UNUSED]], %[[I]]){{.*}} -> ()
// CIR:         cir.return

// LLVM-LABEL: define internal void @"_ZZ11agg_invokervEN3$_08__invokeEi"
// LLVM-SAME:    (ptr {{.*}} sret(%struct.S) {{[^,]*}} %[[RET_ARG:[^,]+]], i32 {{[^,)]*}} %[[I_ARG:[^,)]+]])
// LLVM:         %[[I_ALLOCA:.*]] = alloca i32
// LLVM:         %[[UNUSED:.*]] = alloca %class.anon
// LLVM:         store i32 %[[I_ARG]], ptr %[[I_ALLOCA]]
// LLVM:         %[[I:.*]] = load i32, ptr %[[I_ALLOCA]]
// LLVM:         call void @"_ZZ11agg_invokervENK3$_0clEi"(ptr {{.*}} sret(%struct.S) {{.*}} %[[RET_ARG]], ptr {{.*}} %[[UNUSED]], i32 {{.*}} %[[I]])
// LLVM:         ret void
