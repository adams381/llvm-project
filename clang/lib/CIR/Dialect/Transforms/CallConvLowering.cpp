...
static ArgClassification convertABIArgInfo(const llvm::abi::ABIArgInfo &info,
  ...
  if (origTy)
    if (auto recTy = dyn_cast<cir::RecordType>(origTy)) {
      ...
      if (abi.isEmpty && abi.canPassInRegs)
  ...
  if (auto recTy = dyn_cast<cir::RecordType>(type)) {
    ...
    for (auto it = members.begin(); it != endIt; ++it) {
      ...
      if (auto memberRec = dyn_cast<cir::RecordType>(fieldTy)) {
        if (cir::getRecordABIInfo(module, memberRec).isEmpty &&
    ...
    auto recABI = cir::getRecordABIInfo(module, recTy);
    uint64_t dataSizeBits = recABI.dataSizeInBits;
    ...
    if (nonTrivial) {
      recFlags = recFlags | llvm::abi::RecordFlags::IsCXXRecord;
      recFlags =
          recFlags | llvm::abi::RecordFlags::HasNonTrivialCopyConstructor;
      recFlags = recFlags | llvm::abi::RecordFlags::HasNonTrivialDestructor;
    }
...
classifyWithABILibrary(FunctionOpInterface funcOp, ABITypeMapper &typeMapper,
  ...
  for (Type argTy : funcOp.getArgumentTypes()) {
    const llvm::abi::Type *mapped =
        mapCIRType(argTy, typeMapper, dl, recordCoercionEnabled, module);
