...
// `Task<void>` is a padded struct with no real data members, so the
// calling-convention lowering pass classifies its return as Ignore
// (matching the x86_64 System V ABI's "no class" handling for empty
// records, which OGCG also does).  The `get_return_object()` call
// therefore returns void rather than ![[VoidTask]].
...
// initial_suspend().  `std::suspend_always` is an empty padded struct, so
// the calling-convention lowering pass also Ignores its return value.
...
// CIR:   }, suspend : {
// `coroutine_handle<X>` and `std::suspend_always` are empty records so
// the calling-convention lowering pass classifies their return values
// and any record-by-value parameters as Ignore.  The IR reflects this
// by routing the data through CIRGen's `["ignored"]` alloca for the
// returned coroutine_handle, dropping the second arg from the
// coroutine_handle<void> constructor and from `await_suspend`.
// CIR:     cir.call @_ZNSt16coroutine_handleIN5folly4coro4TaskIvE12promise_typeEE12from_addressEPv(%[[CoroFrameAddr]])
// CIR:     cir.alloca {{.*}} ["ignored"]
...
// `Task<int>` is also an empty padded struct (its `int` field is
// elided in the IR), so its return is also Ignored.
...
// CIR:     }, suspend : {
// Same Ignore behavior for the int variant — `coroutine_handle<X>` and
// `std::suspend_always` returns/by-value args are ignored by the
...
// CIR: cir.cleanup.scope {
// initial_suspend + await(init).  Empty records returned/passed by
// value (suspend_always, coroutine_handle<X>) are Ignored by the
// calling-convention lowering pass — see comments in the same block
// in `silly_task` above.
// CIR:   cir.call @_ZN5folly4coro4TaskIvE12promise_type15initial_suspendEv(%[[PROMISE]]){{.*}}
// CIR:   cir.await(init, ready : {
// CIR:     cir.call @_ZNSt14suspend_always11await_readyEv(%[[SUSP0]]){{.*}}
// CIR:   }, suspend : {
...
// yield_value + await(yield).  Same Ignore comment as above.
// `yield_value` returns `std::suspend_always` which is also Ignored —
// CIRGen's `["ignored"]` alloca holds the (zero-sized) return value
// so we don't reference YIELD_SUSP / READY1 / etc. anymore.
// CIR:   cir.coro.body {
...
// CIR:       cir.call @_ZNSt14suspend_always11await_readyEv(%[[SUSP1]]){{.*}}
// CIR:     }, suspend : {
// CIR:       cir.call @_ZNSt16coroutine_handleIN5folly4coro4TaskIvE12promise_typeEE12from_addressEPv(%{{.*}}){{.*}}
// CIR:       cir.alloca {{.*}} ["ignored"]
...
// return_void + await(final).  Same Ignore comment as above.
// CIR:   cir.call @_ZN5folly4coro4TaskIvE12promise_type13final_suspendEv(%[[PROMISE]]){{.*}}
// CIR:   cir.await(final, ready : {
// CIR:     cir.call @_ZNSt14suspend_always11await_readyEv(%[[SUSP2]]){{.*}}
// CIR:   }, suspend : {
// CIR:     cir.call @_ZNSt16coroutine_handleIN5folly4coro4TaskIvE12promise_typeEE12from_addressEPv(%{{.*}}){{.*}}
// CIR:     cir.alloca {{.*}} ["ignored"]
...
// `Task<int>` is an empty padded struct, so `go(...)` is Ignored on
// the return side too — see comment under `silly_task`.
